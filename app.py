...
def display_data_preview(data, flip_xmcd=False):
    ...
    with col1:
        ...
        return x, y, z, show_energy_trace, col2


def get_step_function_parameters():
...
def get_distribution_parameters(
    ...
    use_seed = st.checkbox("Use `np.random.seed(21)` for reproducibility", value=False)
    if use_seed:
        seed = 21
        np.random.seed(seed)
...
def get_sum_rule_parameters():
    ...
    with col1:
        ...
        nh_dist_variance = st.number_input("Nh Dist Variance", value=0.1, step=0.01)
...
def provide_download_options(results_df):
...
def main():
    ...
    x, y, z, show_energy_trace, plot_column = display_data_preview(data)
    plot_data(x, y, z, show_energy_trace, plot_column)
    ...
    try:
        step_function_plot = create_step_function_plot(
            x, y, z, initial_step1, initial_step2, initial_slope, initial_branching
        )
    ...
    sum_rule_params = get_sum_rule_parameters()
    ...
    # Get sampling size
    sampling_size = get_sampling_size()
    ...
    plot_parameter_distributions(monte_parameters, whole_set)

    # Get additional parameters
    g, c, l_value = get_additional_parameters()
    ...
    # Display results
    results_df = display_results(lz_list, sz_list, mu_tot_list, mu_rat_list, whole_set)

    # Provide download options
    provide_download_options(results_df)
